...
class MainService:
    def make_prediction(self, conds_str):
        ...
        print(table_stats.columns)
        for col in considered_cols:
            if col in col_left:
                feature.append(min_max_normalize(col_left[col], stats.MIN_VAL, stats.MAX_VAL) * 1000)
            else:
                feature.append(
                    min_max_normalize(table_stats.columns[col].min_val(), stats.MIN_VAL, stats.MAX_VAL) * 1000)
            if col in col_right:
                feature.append(min_max_normalize(col_right[col], stats.MIN_VAL, stats.MAX_VAL) * 1000)
            else:
                feature.append(
                    min_max_normalize(table_stats.columns[col].max_val(), stats.MIN_VAL, stats.MAX_VAL) * 1000)
